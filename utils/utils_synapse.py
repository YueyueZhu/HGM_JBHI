...
def calculate_metric_percase(pred, gt):
    ...
    if pred.sum() > 0 and gt.sum()>0:
        ...
        jaccard = metric.binary.jc(pred, gt) #LoU
...
def test_single_volume_V2(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if test_save_path:
        os.makedirs(test_save_path, exist_ok=True)
    if class_names==None:
...
def test_single_volume_V3(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1, class_names=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if test_save_path:
        os.makedirs(test_save_path, exist_ok=True)
    if class_names==None:
